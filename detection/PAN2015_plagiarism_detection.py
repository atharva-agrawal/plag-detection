...
def identify_paraphrase(sent1,sent2):
    ...
    paraphrase = tokenizer.encode_plus(sent1, sent2, return_tensors="pt", truncation=True)
...
def get_entities(txt1, txt2):
    ...
    if entities1.intersection(entities2) == entities1 or entities1.intersection(entities2) == entities2:
        #print(txt1)
        #print(txt2)
        #print(entities1)
        #print(entities2)
...
def serialize_features(susp, src, features, outdir, type_plag, summary_flag, source_text, susp_text):
    """
    ...
    if len(features):
        ...
        for f in features:
            # verbatim
            if type_plag:
                dic = {'susp_id': susp, 'p_type' : 'verbatim', 'string':susp_text[f[0][0]:f[0][1]], 'src_id': src}
                dics.append(dic)
                #count_verbatim += 1
            # summary
            elif summary_flag:
                dic = {'susp_id': susp, 'p_type' : 'summary', 'susp_string':susp_text[f[0][0]:f[0][1]], 'src_id': src, 'source_string': source_text[f[1][0]: f[1][1]]}
                #count_summary += 1
                dics.append(dic)
            else:
                dic = {'susp_id': susp, 'p_type' : 'random or translation', 'susp_string':susp_text[f[0][0]:f[0][1]], 'src_id': src, 'source_string': source_text[f[1][0]: f[1][1]]}
                ...
                for i in comp1:
                    for j in comp2:
                        score = identify_paraphrase(i,j)
                        if score > 0.5:
                            para_pair.append([i,j, score])
        ...
        #write txt file
        if len(dics):
            with open(outdir+str(src)+'-'+str(susp)+'.json', 'w') as file:
                json.dump(dics , file)
            #for d in dics:
...
if __name__ == "__main__":
    ...
    count_verbatim = 0
    count_summary = 0
    random_or_translate = 0

         


   
   # read the .txt file (consisting of machine-generated texts) of your interest and insert it into list
    # data = []
    # with open(FILE) as f: 
    #     contents = f.read()
    #     # split text segments 
    ...
    # for i, (suspicious_file, source_file) in enumerate(pairs):
    #     with open(os.path.join(suspicious_dir, suspicious_file), 'r') as f:
    #         gpt_txt = f.read().strip()  # ✅ This is the actual content to search
    #         print(gpt_txt, "GPT")
...
# This will store all suspicious texts in order
data = []
for i, (suspicious_file, source_file) in enumerate(pairs):
    suspicious_path = os.path.join(suspicious_dir, suspicious_file)
    ...
    # for i in c:
    #     data.append(i.replace("<|startoftext|>","").replace("<|endoftext|>",""))
    ...
    ca_certs="/Users/atharvaagrawal/Documents/MSc Project/plag-ai/elasticsearch-9.0.4/config/certs/http_ca.crt", request_timeout=30, max_retries=10, retry_on_timeout=True)
    ...
    for i in range(len(data)):
        # print(i, count_no, count_verbatim, count_summary, random_or_translate)
        
        # preprocess search queries
        gpt_txt = data[i]
        # gpt_txt = clean_text(gpt_txt)
        # stop_words = set(stopwords.words('english'))
        # word_tokens = nltk.word_tokenize(gpt_txt)
        # filtered_sentence = " ".join([w for w in word_tokens if not w.lower() in stop_words])
        # print(gpt_txt, "filtered sentence")

        print("search start")
        # print("gpt_txt:", gpt_txt[:500])
    ...
    request_timeout=3000
        ...
        if len([hit["_source"] for hit in res['hits']['hits']])>0: elastic_results = [hit["_source"] for hit in res['hits']['hits']]
        else: continue
        for elastic_result in elastic_results:
            #print(elastic_result)
            sgsplag_obj = SGSPLAG(elastic_result['text'], gpt_txt, parameters)
            type_plag, summary_flag = sgsplag_obj.process()
            if len(sgsplag_obj.detections) == 0:
                count_no += 1
            elif len(sgsplag_obj.detections) and type_plag:
                count_verbatim += 1
            elif len(sgsplag_obj.detections) and summary_flag:
                count_summary += 1
            elif len(sgsplag_obj.detections) and not type_plag and not summary_flag:
                random_or_translate+=1    
            serialize_features(elastic_result['doc_id'], i, sgsplag_obj.detections, outdir, type_plag, summary_flag, gpt_txt, elastic_result['text'])
